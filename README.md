...
- **本地修订后导回**：你可以在本地用任意编辑器修订 `docs/*.md` 的正文内容后重新导回；**请务必保留 ZIP 包根目录下的 `config.json` 配置文件**，不要删除。它还原了文档清单、slug 与父子层级，删除后该知识库将无法被正确识别与还原。
- **跨站迁移**：在旧网站用本插件导出知识库为 ZIP，再在新网站用本插件「导入知识库」导入即可完成迁移；同名知识库默认采用「安全覆盖」策略（先建新 → 验证 → 删旧 → 恢复 slug），失败自动回滚、原数据不受影响。
- 导入的文档默认处于**草稿**状态，可在知识库内一键发布后对外可见。
...
| 统计面板（stats） | 仅统计当前用户可访问的资源（总数、公开/私有/已分享/文档数） | 不向普通用户泄露全站私有知识库 / 文档的聚合数量；统计栏可在页面右上角隐藏/恢复（个人选择自动记忆），管理员可在插件设置「控制台知识库管理页显示统计栏」（`showStats`）中为所有用户统一关闭 |
