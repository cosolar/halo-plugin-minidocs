...
MiniDocs 当前主要通过 `minidocsFinder` 向主题提供数据，未内置独立主题路由页面（如 `/kb`）。主题可结合自身页面结构使用：

- **自定义模板 + 页面 slug**：在 Halo 后台创建页面并选择主题自定义模板，用页面 slug 或主题设置（`settings.yaml`）传入 `kbName` / `slug`，再在模板中通过 `minidocsFinder` 取数渲染。
- **全局侧边栏 / 页脚**：`minidocsFinder` 可在任意主题模板位置调用，无需依赖特定路由页面，例如渲染「全站知识库入口」或「文档页脚导航」。
...
<ul>
  <li th:each="kb : ${minidocsFinder.listKnowledgeBases(1, 10).items}">
    <a th:href="'/kb/' + ${kb.metadata.name}" th:text="${kb.spec.displayName}"></a>
    <small th:text="${kb.status.docCount} + ' 篇'"></small>
  </li>
</ul>
```

#### minidocsFinder.getKnowledgeBase(name)
...
| 参数 | 说明 |
| --- | --- |
| `name` | 知识库名称，对应 `KnowledgeBase.metadata.name` |
...
```html
<div th:with="kb = ${minidocsFinder.getKnowledgeBase(kbName)}" th:if="${kb != null}">
  <h1 th:text="${kb.spec.displayName}"></h1>
  <p th:text="${kb.spec.description}"></p>
</div>
```

#### minidocsFinder.listDocs(kbName, page, size)
...
| 参数 | 说明 |
| --- | --- |
| `kbName` | 知识库名称，对应 `KnowledgeBase.metadata.name` |
| `page` | 页码，从 `1` 开始 |
| `size` | 每页条数 |
...
```html
<ul>
  <li th:each="doc : ${minidocsFinder.listDocs(kbName, 1, 20).items}">
    <a th:href="'/docs/' + ${doc.spec.slug}" th:text="${doc.spec.title}"></a>
    <small th:text="${doc.spec.summary}"></small>
  </li>
</ul>
```

#### minidocsFinder.getDoc(kbName, docName)
...
| --- | --- |
| `kbName` | 知识库名称 |
| `docName` | 文档名称，对应 `KnowledgeBaseDoc.metadata.name` |
...
```html
<div th:with="doc = ${minidocsFinder.getDoc(kbName, docName)}" th:if="${doc != null}">
  <h1 th:text="${doc.spec.title}"></h1>
</div>
```

#### minidocsFinder.getDocTree(kbName)
...
文档按 `spec.priority`、创建时间、`metadata.name` 升序；`children` 为子节点列表，可递归渲染。适合侧边栏导航。
...
<nav>
  <ul>
    <li th:each="node : ${minidocsFinder.getDocTree(kbName)}">
      <a th:href="'/docs/' + ${node.slug}" th:text="${node.title}"></a>
      <ul th:if="${node.children != null and !node.children.isEmpty()}">
        <li th:each="child : ${node.children}">
          <a th:href="'/docs/' + ${child.slug}" th:text="${child.title}"></a>
        </li>
      </ul>
    </li>
  </ul>
...
#### minidocsFinder.getDocBySlug(slug)

按 slug 查询已发布文档（所属知识库须公开）。适合「`/docs/{slug}`」这类可读 URL 的详情页。
...
```html
<article th:with="doc = ${minidocsFinder.getDocBySlug(slug)}" th:if="${doc != null}">
  <h1 th:text="${doc.spec.title}"></h1>
  <div id="doc-body" th:attr="data-md=${doc.spec.content}"></div>
</article>
```

> `slug` 变量由你的主题通过「页面 slug 约定」或「主题设置」传入模板。文档 `spec.content` 为**原始 Markdown 文本**，需主题自行渲染（参考下方「Markdown 渲染」）。若希望直接拿到已渲染 HTML，可改用公共 REST 接口的 `contentHtml` 字段。

## Markdown 渲染

`minidocsFinder` 返回的文档 `spec.content` 是原始 Markdown。主题中常用两种渲染方式：
...
```html
<div class="markdown-body" th:utext="${@markdownRender.render(doc.spec.content)}"></div>
```
...
> `content` 为原始 Markdown，`contentHtml` 为服务端渲染后的 HTML（由公共 REST 接口提供，Finder 侧重服务端场景，模板中通常用 `content` 自行渲染）。`phase` 取值：`draft`、`published`。
...
1. **变量名固定**：模板中必须使用 `${minidocsFinder}`，对应插件 `@Finder("minidocsFinder")`，不要猜测其他名字。
