...
主页顶部以统计卡片展示总览（知识库总数、公开数量、私有数量、已分享数量、文档总数），统计栏右上角的眼睛图标可随时隐藏/恢复（个人选择会记住），管理员也可在插件设置中通过「控制台知识库管理页显示统计栏」一键为所有用户关闭；下方是搜索、标签筛选、排序与视图切换，再往下是知识库卡片网格。每张卡片展示封面、名称、标签、权限标签（公开/私有/团队）、文档数、浏览量、点赞数与最后更新时间，卡片底部提供「进入」与右侧操作组（分享/编辑/删除等）。移动端操作统一收进「更多」菜单，标题过长时悬停可查看完整名称。
...
| **封面** | — | 可从本地上传图片或粘贴图片链接 |
| **图标（logo）** | — | 卡片左侧显示的图标，可从本地上传图片或粘贴图片链接；未设置时显示默认书本图标 |
...
在知识库主页点「**导入**」上传的 ZIP，**仅限由本插件导出的知识库包**。导出与导入使用固定的包结构：
...
- **跨站迁移**：在旧网站用本插件导出知识库为 ZIP，再在新网站用本插件「导入」按钮导入即可完成迁移，知识库结构、文档层级与元信息会一并还原。同名知识库默认采用「安全覆盖」策略（先建新 → 验证完整性 → 删旧 → 恢复 slug），失败自动回滚、原数据不受影响。
